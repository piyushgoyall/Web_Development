...
class SideDrawer extends React.Component {
  ...
  changeForm = (event, id) => {
    ...
    upadatedElement.isInValid = this.checkValidity(
      upadatedElement.value,
      upadatedElement.validation
    );
    upadatedElement.touched = true;
    updatedLoginForm[id] = upadatedElement;
    for (let identifier in updatedLoginForm) {
      loginValidationArray = loginValidationArray.concat(
        updatedLoginForm[identifier].isInValid.toString()
      );
    }
    ...
    let loginForm = (
      <form onSubmit={this.login}>
        {loginArray.map((loginForm, key) => {
          return (
            <div key={loginForm.id}>
              <input
                key={loginForm.id}
                type={loginForm.config.elementConfig.type}
                style={
                  loginForm.config.touched && loginForm.config.isInValid
                    ? styles.invalidInput
                    : styles.validInput
                }
        ...
        <Button
          ...
          color="primary"
          style={
            this.state.isInValidLoginForm ? styles.btnDisable : styles.btnColor
          }
          disabled={this.state.isInValidLoginForm}
        >
          Submit
        </Button>
      </form>
      ...
      <Auxillay>
        ...
        <div className="parentDiv">
          <FontAwesomeIcon
            icon="times"
            style={styles.close}
            onClick={this.props.closeSignModal}
          />
          <h2>Login</h2>
