...
class AddressForm extends Component {
  ...
  changeForm = (event, id) => {
    ...
    for (let identifier in updatedAddressForm) {
      validationArray = validationArray.concat(
        updatedAddressForm[identifier].isInValid.toString()
      );
    }
    ...
    let addressForm = (
      <form onSubmit={this.submitAddress}>
          ...
          return (
            <div>
              <input
                type="text"
                style={
                  addressForm.config.touched && addressForm.config.isInValid
                    ? styles.invalidInput
                    : styles.validInput
                }
                key={addressForm.id}
                placeholder={addressForm.config.elementConfig.placeholder}
                onChange={(event) => this.changeForm(event, addressForm.id)}
              />
            </div>
          );
        })}
        ...
        <Button
          ...
          style={
            this.state.isInvalidAddressForm
              ? styles.btnDisable
              : styles.btnColor
          }
